...
const builder_util_3 = require("builder-util");
/** @internal */
async function tar(compression, format, outFile, dirToArchive, isMacApp, tempDirManager) {
    ...
    await Promise.all([
        ...
        (0, builder_util_2.unlinkIfExists)(outFile),
    ]);
    if (format === "tar.lz") {
    ...
    await (0, builder_util_1.exec)(await (0, builder_util_3.getPath7za)(), args, {
...
async function archive(format, outFile, dirToArchive, options = {}) {
    const outFileStat = await (0, builder_util_2.statOrNull)(outFile);
    const dirStat = await (0, builder_util_2.statOrNull)(dirToArchive);
    ...
    await (0, builder_util_2.unlinkIfExists)(outFile);
    ...
    try {
        const binary = use7z ? await (0, builder_util_3.getPath7za)() : "zip";
        await (0, builder_util_1.exec)(binary, args, {
            cwd: options.withoutDir ? dirToArchive : path.dirname(dirToArchive),
        }, builder_util_1.debug7z.enabled);
    }
    catch (e) {
        if (e.code === "ENOENT" && !(await (0, builder_util_2.exists)(dirToArchive))) {
