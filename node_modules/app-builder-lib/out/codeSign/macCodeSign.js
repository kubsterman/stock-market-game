...
async function reportError(isMas, certificateTypes, qualifier, keychainFile, isForceCodeSigning) {
    ...
    if (isMas || isForceCodeSigning) {
        throw new Error(builder_util_3.Logger.createMessage("skipped macOS application code signing", logFields, "error", it => it));
    }
    else {
        builder_util_1.log.warn(logFields, "skipped macOS application code signing");
    }
}
...
const bundledCertKeychainAdded = new lazy_val_1.Lazy(async () => {
    ...
    const results = await Promise.all([
        listUserKeychains(),
        (0, builder_util_2.copyFile)(path.join(__dirname, "..", "..", "certs", "root_certs.keychain"), tmpKeychainPath).then(() => (0, promises_1.rename)(tmpKeychainPath, keychainPath)),
    ]);
...
function removeKeychain(keychainFile, printWarn = true) {
    return (0, builder_util_1.exec)("/usr/bin/security", ["delete-keychain", keychainFile]).catch((e) => {
        ...
        return (0, builder_util_2.unlinkIfExists)(keychainFile);
    });
}
async function createKeychain({ tmpDir, cscLink, cscKeyPassword, cscILink, cscIKeyPassword, currentDir }) {
    ...
    await Promise.all([
        ...
        bluebird_lst_1.default.mapSeries(securityCommands, it => (0, builder_util_1.exec)("/usr/bin/security", it)),
    ]);
...
async function sign(opts) {
    return (0, builder_util_1.retry)(() => (0, osx_sign_1.signAsync)(opts), 3, 5000, 5000);
}
