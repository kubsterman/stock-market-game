...
class MacPackager extends platformPackager_1.PlatformPackager {
    ...
    async sign(appPath, outDir, masOptions, arch) {
        ...
        if (binaries) {
            ...
            binaries = await Promise.all(binaries.map(async (destination) => {
                if (await (0, builder_util_1.statOrNull)(destination)) {
                    return destination;
                }
    ...
    async doSign(opts, customSignOptions) {
        ...
        return customSign ? Promise.resolve(customSign(opts, this)) : (0, macCodeSign_1.sign)(opts);
    }
    //noinspection JSMethodCanBeStatic
    ...
    async applyCommonInfo(appPlist, contentsPath) {
        ...
        if (icon != null) {
            ...
            await (0, builder_util_1.copyFile)(icon, path.join(resourcesPath, iconFileName));
        }
    ...
    getNotarizeOptions(appPath) {
        ...
        const optionsTeamId = options === null || options === void 0 ? void 0 : options.teamId;
        if (optionsTeamId) {
            builder_util_1.log.warn(null, "Please specify notarization Team ID in the `APPLE_TEAM_ID` env var instead of `notarize.teamId`");
