...
class PlatformPackager {
    ...
    constructor(info, platform) {
        ...
        this._resourceList = new lazy_val_1.Lazy(() => (0, builder_util_2.orIfFileNotExist)((0, promises_1.readdir)(this.info.buildResourcesDir), []));
    ...
    async doPack(outDir, appOutDir, platformName, arch, platformSpecificBuildOptions, targets, sign = true, disableAsarIntegrity = false) {
        ...
        if (framework.beforeCopyExtraFiles != null) {
            ...
            await framework.beforeCopyExtraFiles({
                packager: this,
                appOutDir,
                asarIntegrity: asarOptions == null || disableAsarIntegrity ? null : await (0, integrity_1.computeData)({ resourcesPath, resourcesRelativePath }),
                platformName,
            });
        }
        if (this.info.cancellationToken.cancelled) {
    ...
    copyAppFiles(taskManager, asarOptions, resourcePath, defaultDestination, packContext, platformSpecificBuildOptions, excludePatterns, macroExpander) {
        ...
        if (this.info.isPrepackedAppAsar) {
            taskManager.addTask(bluebird_lst_1.default.each(_computeFileSets([new fileMatcher_1.FileMatcher(appDir, resourcePath, macroExpander)]), it => (0, appFileCopier_1.copyAppFiles)(it, this.info, transformer)));
        }
            ...
            taskManager.addTask(bluebird_lst_1.default.each(_computeFileSets(mainMatchers), it => (0, appFileCopier_1.copyAppFiles)(it, this.info, combinedTransformer)));
        }
        else {
    ...
    async computeAsarOptions(customBuildOptions) {
        ...
        if (result === false) {
            const appAsarStat = await (0, builder_util_2.statOrNull)(path.join(this.info.appDir, "app.asar"));
    ...
    async checkFileInPackage(resourcesDir, file, messagePrefix, isAsar, disableSanityCheckAsar) {
        ...
        if (pathParsed.dir.includes(".asar")) {
            ...
            const outStat = await (0, builder_util_2.statOrNull)(fullPath);
            if (outStat == null) {
    ...
    async sanityCheckPackage(appOutDir, isAsar, framework, disableSanityCheckAsar) {
        const outStat = await (0, builder_util_2.statOrNull)(appOutDir);
        if (outStat == null) {
    ...
    computeSafeArtifactName(suggestedName, ext, arch, skipDefaultArch = true, defaultArch, safePattern = "${name}-${version}-${arch}.${ext}") {
        return computeSafeArtifactNameIfNeeded(suggestedName, () => this.computeArtifactName(safePattern, ext, skipDefaultArch && arch === (0, builder_util_2.defaultArchFromString)(defaultArch) ? null : arch));
    }
    expandArtifactNamePattern(targetSpecificOptions, ext, arch, defaultPattern, skipDefaultArch = true, defaultArch) {
        ...
        return this.computeArtifactName(pattern, ext, !isUserForced && skipDefaultArch && arch === (0, builder_util_2.defaultArchFromString)(defaultArch) ? null : arch);
    }
    ...
    computeArtifactName(pattern, ext, arch) {
        const archName = arch == null ? null : (0, builder_util_2.getArtifactArchName)(arch, ext);
    ...
    async getResource(custom, ...names) {
        ...
        if (custom === undefined) {
            ...
            if ((await (0, builder_util_2.statOrNull)(p)) == null) {
